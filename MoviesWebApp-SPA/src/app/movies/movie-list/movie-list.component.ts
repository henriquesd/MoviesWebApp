...
export class MovieListComponent implements OnInit {
  ...
  async getMovies(actualPage) {
      this.http.get('https://localhost:44305/api/movies?page=' + actualPage).subscribe(response => {
  ...
  searchMovieByTitle(value) {
    this.http.get('https://localhost:44305/api/movies/search/' + value + '?page=' + this.actualPage).subscribe(response => {
      this.movies = response['results'];
      this.totalResults = response['total_results'];
    }, error => {
      console.log('An error occurred on get the records.');
